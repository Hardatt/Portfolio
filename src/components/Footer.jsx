...
export default function Footer() {
  ...
  return (
    <footer className={`border-t ${isDark ? 'bg-[#0a0f1e] border-white/5' : 'bg-white border-gray-100'}`}>
            ...
            <div className="flex gap-3">
              {[
                ...
                <motion.a key={label} href={href} target="_blank" rel="noopener noreferrer" aria-label={label}
                  ...
                  className={`w-9 h-9 rounded-xl border flex items-center justify-center transition-all ${isDark ? 'border-white/8 text-gray-400 hover:text-white hover:border-violet-500/50' : 'border-gray-200 text-gray-500 hover:text-gray-900 hover:border-violet-300'}`}>
                  <Icon size={16} />
                </motion.a>
              ))}
            </div>
